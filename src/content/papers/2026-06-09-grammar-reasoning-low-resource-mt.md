...
description: "提出自动化三步流水线将Universal Dependencies树库中的句法标注转化为步骤化语法推理轨迹，系统验证ICL/SFT/RFT三种使用方式对锡伯语和Chintang 语低资源翻译的效果，发现ICL最有效可使chrF提升最高11.89点，总计消耗约2000 GPU小时"
---
...
- Chintang 语（约5,000母语使用者，尼泊尔）：UD 树库2,289句，82条语法规则
...
|----------|------|------|-------|-------|
| Chintang+Gemma-4-E4B | **+5.57** | **+11.89** | **+11.32** | **+18.51** |
| 锡伯+Qwen3-14B | +1.59 | +1.84 | +4.30 | +5.18 |
...
1. **濒危语言保护**：锡伯语（约3万人）、Chintang 语（约5,000人）都濒临消失，本方法在极少数据条件下构建翻译系统，助力语言遗产数字化保护
...
mindmap
  root((语法推理与低资源翻译))
    ...
    三种使用方式
      ICL上下文学习 最有效
        推理轨迹作为少样本示例
        Chintang+Gemma-4-E4B chrF+11.89
      SFT监督微调 效果不一致
        LoRA rank=16
        小模型难以稳定学习
      RFT强化微调 提升有限
