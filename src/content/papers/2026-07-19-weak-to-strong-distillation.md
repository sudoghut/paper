...
现在最先进的推理大模型（比如做数学题很强的模型）大多是靠强化学习（RL）训练出来的。但RL训练有个大麻烦：模型越大，每一次「让模型自己生成答案、再根据答案对错打分调整」的过程就越贵越慢。这就好比让一个刚毕业的博士生天天做模拟考、请一群老师批改再反馈，成本很高；但让一个本科生做同样的模拟考，成本就低得多。
...
这个方法最巧妙的地方在于：它不要求大模型（学生）去模仿小模型（老师）具体说了什么话、用了哪些词，而是学习"RL训练把哪些行为变得更好"这个抽象的改进方向，因此即使学生模型本身的表达习惯和老师完全不同（词汇分布重叠很低），依然能学到进步方向，这就是论文强调的"在完全在策略（on-policy）的状态下进行知识迁移"，避免了传统蒸馏方法要求师生模型输出高度重合才有效的限制。
