...
2. **给每个函数打两个分数**：
   ...
   - **可推断性分数（Î）**：综合五个线索——调用者传参数的具体程度、这个函数自己的"扇出"（fan-out，指它往外调用了同文件里多少个别的函数，而不是它被多少地方调用；一个函数如果调用了很多已知的辅助函数，会比只调用外部黑盒库的孤立函数更容易被合理推断出来）、类型注解、文档字符串（docstring）、类之间的耦合程度，代表"根据上下文，这个函数能不能被合理地猜出来"。
